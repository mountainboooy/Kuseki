...
- (void)viewDidLoad {
  ...
  // button_close
  [_btClose_train addTarget:self
                     action:@selector(btCloseTrainPressed)
           forControlEvents:UIControlEventTouchUpInside];
  [_btClose_dep addTarget:self
                   action:@selector(btCloseDepPressed)
         forControlEvents:UIControlEventTouchUpInside];
  [_btClose_arr addTarget:self
                   action:@selector(btCloseArrPressed)
         forControlEvents:UIControlEventTouchUpInside];

  // button_next
  [_btNext_dep addTarget:self
                  action:@selector(btNextDepPressed)
        forControlEvents:UIControlEventTouchUpInside];
  [_btSearch_arr addTarget:self
                    action:@selector(btSearchArrPressed)
          forControlEvents:UIControlEventTouchUpInside];
...
- (void)updateCell:(UITableViewCell *)cell
  ...
  case 3: { //乗車駅・降車駅
    ...
    [bt_reverse addTarget:self
                   action:@selector(btReversePressed)
         forControlEvents:UIControlEventTouchUpInside];

    //セル選択用のボタン
    UIButton *bt_dep = (UIButton *)[cell viewWithTag:3];
    UIButton *bt_arr = (UIButton *)[cell viewWithTag:4];
    [bt_dep addTarget:self
                  action:@selector(btDepPressed)
        forControlEvents:UIControlEventTouchUpInside];
    [bt_arr addTarget:self
                  action:@selector(btArrPressed)
        forControlEvents:UIControlEventTouchUpInside];
    ...
    if (_selected_index == 3) { //乗車駅
      ...
      focus_view.frame =
          CGRectMake(0, 0, cell.bounds.size.width, cell.bounds.size.height / 2);

    } else if (_selected_index == 4) { //降車駅
      focus_view.alpha = 0.2;
      focus_view.frame =
          CGRectMake(0, cell.bounds.size.height / 2, cell.bounds.size.width,
  ...
  case 4: {
    ...
    [bt_search addTarget:self
                  action:@selector(btSearchPressed)
        forControlEvents:UIControlEventTouchUpInside];
    break;
  }
  }
...
- (void)textFieldDidBeginEditing:(MITextField *)textField {
  ...
  //テーブルをスクロール
  [_tableView scrollToRowAtIndexPath:textField.indexPath
...
- (void)textFieldDidEndEditing:(MITextField *)textField {
  ...
  UITableViewCell *cell =
      [_tableView cellForRowAtIndexPath:textField.indexPath];
...
- (BOOL)textField:(MITextField *)textField
  ...
  case 0: {                   //乗車月日
    if (textField.tag == 1) { //月
      ...
      if (text.length == 1 && text.intValue > 1) {
        ...
        [self performSelector:@selector(focusNextField:)
  ...
  case 1: {                   //時間
    if (textField.tag == 1) { //時
      ...
      if (text.length == 1 && text.intValue > 5) {
        ...
        textField.text = @"0";
        [self performSelector:@selector(showPickerTrain)
                   withObject:nil
                   afterDelay:0.1];
        return YES;
      }
...
- (void)initCondition {
  ...
  NSDateComponents *dateComps = [calendar
...
- (IBAction)btBackTrainPressed:(id)sender {
  ...
  UITableViewCell *previousCell =
      [_tableView cellForRowAtIndexPath:previousPath];
...
- (void)btSearchPressed {
  ...
  ResultsViewController *resultCon = [self.storyboard
      instantiateViewControllerWithIdentifier:@"ResultsViewController"];
...
- (BOOL)isValidTime:(NSDate *)date {
  ...
  if (!date) {
    [[NSException exceptionWithName:@"TimeValudateionExecption"
                             reason:@"date is null"
                           userInfo:nil] raise];
  }
...
- (IBAction)btInfomationPressed:(id)sender {
  ...
  InformationViewController *infoCon = [self.storyboard
      instantiateViewControllerWithIdentifier:@"InformationViewController"];
