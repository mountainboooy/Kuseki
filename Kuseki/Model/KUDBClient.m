...
- (void)createTableWithName:(enum KUTableName)tableName{
    ...
    if (tableName == KU_TABLE_CONDITIONS) {//検索条件
        ...
        sql = @"CREATE TABLE IF NOT EXISTS notification_targets (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, dep_time TEXT, arr_time TEXT, seat_ec_ns TEXT, seat_ec_s TEXT, seat_gr_ns TEXT, seat_gr_s TEXT, condition_id TEXT, dep_stn TEXT, arr_stn TEXT, month TEXT, day TEXT )";
    }
...
- (void)insertNotificationTarget:(KUNotificationTarget*)target
{
    ...
    FMDatabase *db = [FMDatabase databaseWithPath:_dbPath];
    NSString *sql = @"INSERT INTO notification_targets(name, dep_time, arr_time, seat_ec_ns, seat_ec_s, seat_gr_ns, seat_gr_s, condition_id, dep_stn, arr_stn, month, day) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    [db open];
    [db executeUpdate:sql, target.name, target.dep_time, target.arr_time, ec_ns, ec_s, gr_ns, gr_s, target.condition_id , target.dep_stn, target.arr_stn, target.month, target.day];
...
- (void)updateNotificationtarget:(KUNotificationTarget*)target{
    ...
    FMDatabase *db = [FMDatabase databaseWithPath:_dbPath];
    NSString *sql = @"UPDATE notification_targets SET name = ?, dep_time = ?, arr_time = ?, seat_ec_ns = ?, seat_ec_s = ?, seat_gr_ns = ?, seat_gr_s = ?, condition_id = ?, dep_stn = ?, arr_stn = ?, month = ?, day = ? WHERE id = ? VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    
    
    [db open];
    [db executeUpdate:sql, target.name, target.dep_time, target.arr_time, target.seat_ec_ns, target.seat_ec_s, target.seat_gr_ns, target.seat_gr_s, target.condition_id, target.dep_stn, target.arr_stn, target.month, target.day];
